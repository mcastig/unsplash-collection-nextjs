import searchReducer, { setQuery, clearResults, searchPhotos } from './searchSlice';
...
describe('searchSlice — reducers', () => {
  ...
  it('clearResults resets search state', () => {
    const dirty = { ...initialState, query: 'x', results: [{} as never], hasSearched: true };
...
describe('searchSlice — async thunks', () => {
  ...
  it('sets loading on pending', () => {
    const state = searchReducer(initialState, { type: searchPhotos.pending.type });
  ...
  it('stores results on fulfilled', () => {
    ...
    const action = { type: searchPhotos.fulfilled.type, payload, meta: { arg: { page: 1 } } };
  ...
  it('stores error on rejected', () => {
    const action = { type: searchPhotos.rejected.type, error: { message: 'fail' } };
    const state = searchReducer(initialState, action);
    expect(state.loading).toBe(false);
  ...
  it('dispatches fulfilled when fetch succeeds', async () => {
    ...
    expect(dispatch).toHaveBeenCalledWith(expect.objectContaining({ type: searchPhotos.fulfilled.type }));
  });
  ...
  it('dispatches rejected when fetch fails', async () => {
    ...
    expect(dispatch).toHaveBeenCalledWith(expect.objectContaining({ type: searchPhotos.rejected.type }));
  });
});
