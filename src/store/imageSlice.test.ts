...
const mockImage = { id: 'abc', created_at: '2024-01-01', width: 800, height: 600, description: null, alt_description: null, urls: { raw: '', full: '', regular: '', small: '', thumb: '' }, links: { html: '', download: '', download_location: '' }, user: { id: 'u1', name: 'Test', username: 'test', profile_image: { small: '', medium: '', large: '' }, links: { html: '' } } };
...
describe('imageSlice — reducers', () => {
  ...
  it('clearImage resets state', () => {
    const dirty = { ...initialState, currentImage: mockImage as never, imageCollections: [{} as never] };
  ...
  it('removeCollectionFromImage filters by collectionId', () => {
    ...
    const s = imageReducer({ ...initialState, imageCollections: [entry] }, removeCollectionFromImage(1));
...
describe('imageSlice — async thunks', () => {
  it('sets loading on fetchImageDetail pending', () => {
    const s = imageReducer(initialState, { type: fetchImageDetail.pending.type });
    expect(s.loading).toBe(true);
  });

  it('stores image on fetchImageDetail fulfilled', () => {
    const s = imageReducer(initialState, { type: fetchImageDetail.fulfilled.type, payload: mockImage });
  ...
  it('stores imageCollections on fetchImageCollections fulfilled', () => {
    ...
    const s = imageReducer(initialState, { type: fetchImageCollections.fulfilled.type, payload });
