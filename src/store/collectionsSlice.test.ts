...
const col: Collection = { id: 1, name: 'Nature', created_at: '2024-01-01', image_count: 2, cover_image: null };
...
describe('collectionsSlice — reducers', () => {
  it('returns initial state', () => {
    expect(collectionsReducer(undefined, { type: '@@INIT' })).toEqual(initialState);
  });

  it('clearSelectedCollection resets selected + images', () => {
    const state = collectionsReducer(
      { ...initialState, selectedCollection: col, collectionImages: [{} as never] },
      clearSelectedCollection()
    );
...
describe('collectionsSlice — fetchCollections', () => {
  it('sets loading on pending', () => {
    const s = collectionsReducer(initialState, { type: fetchCollections.pending.type });
  ...
  it('stores error on rejected', () => {
    const s = collectionsReducer(initialState, { type: fetchCollections.rejected.type, error: { message: 'err' } });
...
describe('collectionsSlice — removeImageFromCollection', () => {
  it('removes image from collectionImages', () => {
    const img = { id: 1, collection_id: 1, image_id: 'img1', image_url: '', image_thumb_url: '', image_small_url: '', photographer_name: '', photographer_username: '', photographer_avatar: '', published_at: null, created_at: '' };
    const action = { type: removeImageFromCollection.fulfilled.type, payload: { collectionId: 1, imageId: 'img1' } };
    const s = collectionsReducer({ ...initialState, collectionImages: [img] }, action);
...
describe('collectionsSlice — thunk integration', () => {
  ...
  it('fetchCollections calls /api/collections', async () => {
    (global.fetch as jest.Mock).mockResolvedValueOnce({ ok: true, json: async () => [col] });
  ...
  it('deleteCollection calls DELETE /api/collections/:id', async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith('/api/collections/1', { method: 'DELETE' });
  });
});
